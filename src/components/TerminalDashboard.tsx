...
export function TerminalDashboard() {
  ...
  const startAudit = async () => {
    ...
    try {
      ...
      await runAudit({ scanId, sessionToken: token });
    } catch (caught) {
      const message = caught instanceof Error ? caught.message : "Unable to start the audit.";
      setError(message.includes("not publicly accessible") ? "This repository is private or unavailable. Make it public on GitHub, then paste the public repository link." : message);
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <div className="workspace-shell">
      ...
      <div className="border-b border-white/[0.06] p-5 md:p-7">
        ...
        <div className="mt-3 flex flex-col justify-between gap-2 text-xs text-[#62666d] sm:flex-row">
          <span>{repoLabel}</span>
      ...
      <div className={`grid transition-[min-height] ${scan ? "min-h-[610px]" : "min-h-[430px]"} lg:grid-cols-[220px_1fr_370px]`}>
        <aside className="border-b border-white/[0.06] p-4 lg:border-b-0 lg:border-r">
          <div className="mb-4 flex items-center gap-2 px-2 text-[11px] uppercase tracking-[0.14em] text-[#62666d]"><History size={13} /> This tab</div>
          <div className="flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible">
            {recentScans?.length ? recentScans.map((item) => (
              <button key={item._id} onClick={() => setActiveScanId(item._id)} className={`min-w-44 rounded-lg border px-3 py-3 text-left transition lg:min-w-0 ${selectedScanId === item._id ? "border-[#7170ff]/40 bg-[#7170ff]/[0.08]" : "border-transparent hover:bg-white/[0.03]"}`}>
                ...
                <div className="mt-2 flex items-center justify-between text-[10px] text-[#62666d]"><span>{statusCopy[item.status]}</span><ChevronRight size={12} /></div>
              </button>
            )) : <p className="px-2 text-xs leading-5 text-[#62666d]">Your scans appear here after you run one.</p>}
          </div>
        </aside>
        ...
        <section className="border-b border-white/[0.06] p-5 lg:border-b-0 lg:border-r lg:p-6">
          <div className="mb-4 flex items-center justify-between border-b border-white/[0.06] pb-4">
            ...
            <span className={`rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-wider ${scan?.status === "completed" ? "border-[#43c887]/30 text-[#72dca0]" : scan?.status === "failed" ? "border-[#ff7272]/30 text-[#ff9a9a]" : "border-[#7170ff]/30 text-[#9da4ff]"}`}>{scan ? statusCopy[scan.status] : "ready"}</span>
          </div>
          <div ref={logViewport} className={`${scan ? "h-[460px]" : "h-[280px]"} overflow-y-auto pr-2 font-mono text-xs leading-6 transition-[height]`}>
            {logs?.length ? logs.map((log, index) => (
              <div key={log._id} className="agent-log flex gap-3" style={{ animationDelay: `${Math.min(index, 12) * 35}ms` }}>
                <span className="w-28 shrink-0 truncate text-[#62666d]">{log.agent}</span>
            ...
            )) : <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-[#62666d]"><CircleDotDashed size={24} /><span>Paste any public GitHub repository and launch the crew.</span></div>}
          </div>
        </section>

        <aside className="p-5 lg:p-6">
          <div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.14em] text-[#62666d]"><ShieldAlert size={14} /> Findings</div><span className="text-2xl font-medium text-[#f7f8f8]">{findings?.length ?? "—"}</span></div>
          <div className="max-h-[520px] space-y-3 overflow-y-auto pr-1">
            {findings?.length ? findings.map((finding) => (
              <article key={finding._id} className="finding-card">
                <div className="flex items-center justify-between gap-3"><SeverityBadge severity={finding.severity} /><span className="truncate text-[10px] text-[#62666d]">{finding.filePath}</span></div>
                ...
                {finding.status === "open" && scan?.status === "completed" && <button onClick={() => { setRiskFindingId(finding._id); setRiskReason(""); setRiskError(""); }} className="mt-3 text-[10px] text-[#8a8f98] hover:text-[#e8b84c]">Document accepted risk</button>}
            ...
            )) : <div className="flex min-h-56 flex-col items-center justify-center gap-3 text-center text-xs leading-5 text-[#62666d]"><CircleDotDashed size={24} /><span>No findings in the active scan.</span></div>}
          </div>
        </aside>
      </div>
